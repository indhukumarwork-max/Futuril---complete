...
## 2. Onboarding, Interest Discovery & Career DNA **(MVP)**
...
| POST | `/discovery/sessions` | Start a discovery_session |
| GET | `/discovery/sessions/:id/next-activity` | Returns the next `discovery_activity_template` for this session (engine decides order/adaptivity) |
| POST | `/discovery/sessions/:id/responses` | Submit a `discovery_response` for the current activity |
| POST | `/discovery/sessions/:id/complete` | Marks session complete, triggers Career DNA generation |
| POST | `/career-dna/generate` | Analysis Engine: reads all `discovery_response` rows for the session, generates `career_dna` + full `dna_node` set (versioned) |
| GET | `/career-dna/latest` | Current Career DNA summary + all `dna_node` rows, ordered by `helix_position` |
| GET | `/career-dna/nodes/:id` | Single node detail (title, description, confidence, AI interpretation, supporting evidence) — powers the click-to-zoom helix interaction |
| GET | `/career-dna/history` | Full evolution timeline |
| POST | `/career-dna/re-evaluate` | Re-run after milestones (triggers new version + new node set, "Your Career DNA has evolved") |

Note: `/discovery/sessions/:id/next-activity` is intentionally engine-driven rather than a fixed list — new activity types (per `discovery_activity_template.type`) can be added without changing this endpoint's contract, per the feature spec's modularity requirement.
